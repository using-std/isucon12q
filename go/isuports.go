...
func createTenantDB(id int64) error {
	...
	// index追加分
	cmd = exec.Command("sh", "-c", fmt.Sprintf("sqlite3 %s < %s", p, tenantDBSchemaFilePath2))
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to exec sqlite3 %s < %s, out=%s: %w", p, tenantDBSchemaFilePath, string(out), err)
	}
	return nil
...
func playersAddHandler(c echo.Context) error {
	...
	for _, displayName := range displayNames {
		...
		now := time.Now().Unix()
		players = append(players, PlayerRow{ID: id, TenantID: v.tenantID, DisplayName: displayName, IsDisqualified: false, CreatedAt: now, UpdatedAt: now})
		pds = append(pds, PlayerDetail{
			ID:             id,
			DisplayName:    displayName,
...
func competitionScoreHandler(c echo.Context) error {
	...
	if _, err := tenantDB.NamedExecContext(
		...
		return fmt.Errorf(
			...
			playerScoreRows[0].ID, playerScoreRows[0].TenantID, playerScoreRows[0].PlayerID, playerScoreRows[0].CompetitionID, playerScoreRows[0].Score, playerScoreRows[0].RowNum, playerScoreRows[0].CreatedAt, playerScoreRows[0].UpdatedAt, err,
		)
	}
...
func playerHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(ctx, &pss, "SELECT tenant_id, id, player_id, competition_id, score, row_num,created_at, updated_at FROM (SELECT *, RANK () OVER (PARTITION BY competition_id  ORDER BY row_num DESC) AS rank FROM player_score WHERE tenant_id = ? AND player_id = ?) WHERE rank = 1", v.tenantID, p.ID); err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
